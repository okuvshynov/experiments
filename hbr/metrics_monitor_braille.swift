...
class MemoryMonitor {
    func readMemoryInfo() -> (total: UInt64, used: UInt64, wired: UInt64)? {
        ...
        return (total: totalMemory, used: 0, wired: wiredMemory)
    }
}

// MARK: - Braille Chart Creation

func createHorizontalBrailleChart(_ values: [Double], width: Int) -> String {
    // Braille pattern base (8-dot)
    let BRAILLE_BASE = 0x2800
    
    // Individual dot patterns for each line
    let LINE_LEFT_DOTS = [
        0x01,  // dot 1 (line 0, top)
        0x02,  // dot 2 (line 1)
        0x04,  // dot 3 (line 2)
        0x40,  // dot 7 (line 3, bottom)
    ]
    
    let LINE_RIGHT_DOTS = [
        0x08,  // dot 4 (line 0, top)
        0x10,  // dot 5 (line 1)
        0x20,  // dot 6 (line 2)
        0x80,  // dot 8 (line 3, bottom)
    ]
    
    var chars: [Character] = []
    
    for charPos in 0..<width {
        // Calculate two positions within this character
        let leftPos = Double(charPos * 2) / Double(width * 2 - 1)
        let rightPos = Double(charPos * 2 + 1) / Double(width * 2 - 1)
        
        // Build the Braille character
        var dots = 0
        for (j, value) in values.enumerated() {
            if j < 4 {  // Only process first 4 values
        ...
        let char = Character(UnicodeScalar(BRAILLE_BASE + dots)!)
        chars.append(char)
    }
...
// Calculate metrics for Braille chart
var metrics: [Double] = []

// 1. Performance cores utilization
metrics.append(cpuLoad?.usagePCores ?? 0.0)

// 2. Efficiency cores utilization
metrics.append(cpuLoad?.usageECores ?? 0.0)

// 3. GPU utilization
metrics.append(gpuUtilization)

// 4. Wired memory ratio
if let memory = memoryInfo {
    let wiredRatio = Double(memory.wired) / Double(memory.total)
    metrics.append(wiredRatio)
} else {
    metrics.append(0.0)
}
