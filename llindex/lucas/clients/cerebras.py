...
class CerebrasClient:
    def __init__(self, tokens_rate=5000, period=5, max_tokens=4096, model='llama3.1-70b'):
    ...
    def query(self, context: ChunkContext):
        ...
        if wait_for > 0:
            context.metadata['cerebras_wait'] = wait_for
            logging.info(f'cerebras client-side rate-limiting. Waiting for {wait_for} seconds')
            time.sleep(wait_for)
