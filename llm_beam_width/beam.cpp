...
int run(llama_model * model, llama_context * ctx, size_t idx)
{
    ...
    for (size_t i = 0; i < in_tokens.size(); i++)
    {
        ...
        llama_batch_add(batch, in_tokens[i], i, { 1 }, false);
    }

    batch.logits[batch.n_tokens - 1] = true;
    batch.logits[batch.n_tokens - 2] = true;
    ...
    std::map<size_t, uint64_t> ranks1;
    ...
    for (llama_token true_id : main_tokens)
    {
        ...
        rank = 0;
        logits = llama_get_logits_ith(ctx, batch.n_tokens - 2);
        for (llama_token token_id = 0; token_id < n_vocab; token_id++)
        {
            if (logits[token_id] > logits[true_id])
            {
    ...
    for (auto it : ranks1)
    {
        std::cout << "  n_rank1[" << it.first << "] = " << it.second << std::endl;
    }
