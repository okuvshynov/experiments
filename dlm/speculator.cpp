...
class speculator
{
  ...
  private:
    ...
    void eval_loop();
    int  speculate(const std::vector<llama_token> & prompt);
    bool merge_speculation(
            std::vector<llama_token> & curr,
            size_t                   & n_matched);
    ...
    size_t                   & n_matched)
{
    ...
    curr      = res_j["spec"].get<std::vector<llama_token>>();
    return false;
}

// Continuous speculation on single prompt
// TODO: if running indefinitely, this will get out of bounds
int speculator::speculate(const std::vector<llama_token> & prompt)
{
    ...
    std::vector<llama_token> curr = prompt;
